...
class BNFeatureHook:
    def __init__(self, module):
        ...
        self.r_feature = None  # 确保初始化 r_feature 为 None

    def hook_fn(self, module, input, output):
        """
        该函数会在前向传播时被调用，计算并保存 r_feature
        """
        nch = input[0].shape[1]
        mean = input[0].mean([0, 2, 3])  # 计算均值
        var = input[0].permute(1, 0, 2, 3).contiguous().reshape([nch, -1]).var(1, unbiased=False)  # 计算方差
        r_feature = torch.norm(module.running_var.data - var, 2) + torch.norm(module.running_mean.data - mean, 2)
        self.r_feature = r_feature  # 更新 r_feature
...
#     def __init__(self, module):
#         self.hook = module.register_forward_hook(self.hook_fn)
#         print(f"Registered hook for layer: {module}")
...
def save_images_ufc(args, images, targets, ipc_id, model_index):
    for id in range(images.shape[0]):#90
        if targets.ndimension() == 1:
            class_id = targets[id].item()
        else:
            class_id = targets[id].argmax().item()
