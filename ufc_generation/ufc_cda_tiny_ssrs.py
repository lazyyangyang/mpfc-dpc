...
def ssrs_lr_scheduler(optimizer, epoch, total_epochs, ζ):
    """
    Smoothing-later Steep Learning Rate Schedule (SSRS)
    :param optimizer: The optimizer to apply the learning rate schedule.
    :param epoch: Current epoch.
    :param total_epochs: Total number of epochs.
    :param ζ: Deceleration factor to control the rate of smoothing.
    """
    if epoch <= (5 * total_epochs / 6):
        lr = (1 + np.cos(epoch * np.pi / (ζ * total_epochs))) / 2
    else:
        lr = (1 + np.cos(5 * np.pi / (ζ * 6))) * (6 * total_epochs - 6 * epoch) / (6 * total_epochs)
...
def get_images(args, model_lists, hook_for_display, ipc_id):
    ...
    for kk in range(0, args.num_class, batch_size):
        ...
        # 使用SSRS策略替代原本的lr_scheduler
        def lr_scheduler(epoch):
            ssrs_lr_scheduler(optimizer, epoch, args.iteration, ζ=2)  # 设置合适的ζ值
        
        criterion = nn.CrossEntropyLoss().cuda()
    
        # 确保DataParallel应用在CUDA之前
        ...
        for iteration in range(args.iteration):
            ...
            min_crop = 0.08
            max_crop = 1.0
            if iteration < args.milestone * args.iteration:
                if args.easy2hard_mode == "step":
                    min_crop = 1.0
                elif args.easy2hard_mode == "linear":
                    min_crop = 0.08 + (1.0 - 0.08) * (1 - iteration / (args.milestone * args.iteration))
                elif args.easy2hard_mode == "cosine":
                    min_crop = 0.08 + (1.0 - 0.08) * (1 + np.cos(np.pi * iteration / (args.milestone * args.iteration))) / 2
        ...
        if args.store_best_images:
            best_inputs = inputs.data.clone()  # 使用多重裁剪，保存最后一个
