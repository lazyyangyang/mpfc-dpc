...
def get_images(args, model_lists, hook_for_display, ipc_id):
    ...
    for kk in range(0, args.num_class, batch_size):
        ...
        for iteration in range(iterations_per_layer):
            ...
            # cd_loss = cosine_diversity_loss(
            #     uni_perb,
            #     resnet_model.avgpool
            # )

            # cdm_loss = cosine_distribution_matching_loss(
            #     uni_perb,
            #     input_original,
            #     resnet_model.avgpool
            # )

            # edm_loss = euclidean_distribution_matching_loss(
            #     uni_perb,
            #     input_original,
            #     resnet_model.avgpool
            # )

            total_loss = loss_ce + loss_aux 
# + cd_loss + cdm_loss + edm_loss
    ...
    dataset_models = {
        'cifar10': {
            ...
            'model_types': [
                ...
                MobileNetV2_cifar10,
                EfficientNetB0_cifar10,
                ShuffleNetG2_cifar10
            ],
            'model_paths': [
...
def get_args():
    parser = argparse.ArgumentParser(
        description="UFC: Generate Inter-class Feature Compensator"
    )
    ...
    parser.add_argument(
        ...
        default=4,
        type=int,
        help="Number of architectures involved in UFC generation"
    )
    ...
    args.syn_data_path = os.path.join(
        args.syn_data_path,
        args.wandb_name,
        args.exp_name
    )
...
if __name__ == "__main__":
    ...
    args.ipc_init：这个值的计算方式是：args.ipc / (args.M / args.num_class + 1)，
    这会将每个类别的图像数 (args.ipc) 分配到 args.M 个模型中，然后得到每个模型的图像数。

    args.ipc_end：ipc_end 是通过将 ipc_init 乘以 (args.M + 1) 来计算的。
    """
