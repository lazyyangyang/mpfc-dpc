...
def cosine_diversity_loss(synthetic_data, feature_extractor):
    """
    Cosine Diversity Loss (CD): Encourages synthetic samples to be more diverse.
    :param synthetic_data: The synthetic images tensor of shape [batch_size, 3, 64, 64].
    :param feature_extractor: Pre-trained feature extractor network (e.g., ResNet).
    """
    # 将图像数据传入预训练的特征提取器（例如，ResNet的avgpool层），
    # 得到 [batch_size, feature_size] 的特征表示
    embeddings = feature_extractor(synthetic_data)  # [batch_size, feature_size]
    
    # 展平嵌入特征，以便计算余弦相似度
    embeddings = embeddings.view(embeddings.size(0), -1)  # Flatten to [batch_size, feature_size]
    
    # 计算余弦相似度矩阵
    cosine_sim = torch.matmul(embeddings, embeddings.T)  # 计算余弦相似度
    
    # 计算余弦相似度的损失
    cosine_div_loss = 1 - cosine_sim.mean()  # 平均值越小表示多样性越高
    return cosine_div_loss

# Cosine Distribution Matching Loss (CDM)
def cosine_distribution_matching_loss(synthetic_data, real_data, feature_extractor):
    """
    Cosine Distribution Matching Loss (CDM): Encourages synthetic data directions to align with real data.
    """
    # 计算合成数据和真实数据的嵌入表示
    synthetic_embeddings = feature_extractor(synthetic_data)  # [batch_size, feature_size]
    real_embeddings = feature_extractor(real_data)  # [batch_size, feature_size]
    
    # 确保 embeddings 的形状是 [batch_size, feature_size]
    synthetic_embeddings = synthetic_embeddings.view(synthetic_embeddings.size(0), -1)  # Flatten
    real_embeddings = real_embeddings.view(real_embeddings.size(0), -1)  # Flatten
    
    # 计算合成数据和真实数据之间的余弦相似度
    cosine_sim = F.cosine_similarity(synthetic_embeddings, real_embeddings, dim=1)  # 计算每一对样本的余弦相似度
    
    # 计算余弦相似度的损失
    cdm_loss = 1 - cosine_sim.mean()  # 平均值越小表示合成数据和真实数据对齐得越好
    return cdm_loss

# Euclidean Distribution Matching Loss (EDM)
def euclidean_distribution_matching_loss(synthetic_data, real_data, feature_extractor):
    """
    Euclidean Distribution Matching Loss (EDM): Encourages synthetic data to cluster near real data.
    """
    # 计算合成数据和真实数据的嵌入表示
    synthetic_embeddings = feature_extractor(synthetic_data)
    real_embeddings = feature_extractor(real_data)
    
    # 计算合成数据和真实数据之间的欧几里得距离
...
def get_images(args, model_lists, hook_for_display, ipc_id):
    ...
    for kk in range(0, args.num_class, batch_size):
        ...
        feature_extractor = resnet_model.avgpool  # 使用 avgpool 层作为特征提取器
        ...
        for iteration in range(iterations_per_layer):
            ...
            for idx, mod in enumerate(loss_r_feature_layers):
                if mod.r_feature is not None:
                    ...
                    print(f"Warning: r_feature for module {idx} is None.")
            loss_r_bn_feature = torch.stack(loss_r_bn_feature).sum() if loss_r_bn_feature else 0

            loss_aux = args.r_bn * loss_r_bn_feature

            # Add DiRe losses (Cosine Diversity, Cosine Distribution Matching, and Euclidean Matching)
            cd_loss = cosine_diversity_loss(uni_perb, feature_extractor)
            cdm_loss = cosine_distribution_matching_loss(uni_perb, inputs_ori, feature_extractor)
