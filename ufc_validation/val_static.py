...
model_classes = {
    ...
    "mobilenetV2": MobileNetV2_cifar100 if args.dataset == "tiny" else MobileNetV2_cifar10,
    "efficientnet": EfficientNetB0_cifar100 if args.dataset == "tiny" else EfficientNetB0_cifar10,
    "shufflenet": ShuffleNetG2_cifar100 if args.dataset == "tiny" else ShuffleNetG2_cifar10
...
with torch.no_grad():
    for inputs, _, indices in trainloader:
        ...
        print(f"len(model_types): {len(model_types)}, len(model_lists): {len(model_lists)}")
        ...
        # soft_label = torch.mean(torch.stack(soft_label_avg), dim=0)
