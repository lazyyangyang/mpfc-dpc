...
parser.add_argument("--tsne", action="store_true", help="Save a t-SNE analysis figure after the final evaluation")
parser.add_argument("--tsne-output-dir", default="./tsne", type=str, help="Directory to save t-SNE figures")
parser.add_argument("--tsne-samples-per-class", default=20, type=int, help="Maximum samples per class for each domain in t-SNE")
parser.add_argument("--tsne-max-samples", default=2000, type=int, help="Maximum samples for each domain in t-SNE; use <=0 to disable")
parser.add_argument("--tsne-perplexity", default=30.0, type=float, help="t-SNE perplexity before automatic clipping")
parser.add_argument("--tsne-random-state", default=0, type=int, help="Random seed for t-SNE")
...
def _last_linear_layer(model):
    ...
    if last_linear is None:
        raise RuntimeError("Cannot find a Linear classifier layer for t-SNE feature extraction.")
    return last_linear


def _collect_tsne_features(loader, domain_name):
    model_student.eval()
    features = []
    labels = []
    domains = []
    ...
    handle = _last_linear_layer(model_student).register_forward_hook(hook_fn)
    try:
        with torch.no_grad():
            for batch in loader:
                inputs, targets = batch[0], batch[1]
                inputs = inputs.to(device)
                _ = model_student(inputs)
                if "features" not in captured:
                    raise RuntimeError("t-SNE feature hook did not capture classifier inputs.")

                batch_features = captured.pop("features")
                batch_features = batch_features.view(batch_features.size(0), -1).cpu().numpy()
                targets = targets.cpu().numpy()

                for feature, target in zip(batch_features, targets):
                    class_id = int(target)
                    if args.tsne_samples_per_class > 0 and class_counts[class_id] >= args.tsne_samples_per_class:
                        continue
                    ...
                    collected += 1
                    if args.tsne_max_samples > 0 and collected >= args.tsne_max_samples:
                        break
                if args.tsne_max_samples > 0 and collected >= args.tsne_max_samples:
                    break
    finally:
    ...
    if not features:
        raise RuntimeError(f"No samples were collected for {domain_name} t-SNE.")
    return np.asarray(features), np.asarray(labels), np.asarray(domains)


def run_tsne_analysis(epoch):
    print("==> Running t-SNE analysis...")
    synthetic_features, synthetic_labels, synthetic_domains = _collect_tsne_features(trainloader, "synthetic")
    real_features, real_labels, real_domains = _collect_tsne_features(testloader, "real")

    features = np.concatenate([synthetic_features, real_features], axis=0)
    labels = np.concatenate([synthetic_labels, real_labels], axis=0)
    domains = np.concatenate([synthetic_domains, real_domains], axis=0)

    if features.shape[0] < 3:
        print("Skip t-SNE: fewer than 3 collected samples.")
        return
    ...
    output_name = f"{args.dataset}_{args.networks}_ipc{args.ipc}_epoch{epoch}_tsne.png"
    output_path = os.path.join(args.tsne_output_dir, output_name)

    plt.figure(figsize=(9, 7), dpi=180)
    for domain_name, marker, alpha in [("synthetic", "o", 0.85), ("real", "x", 0.65)]:
        ...
        plt.scatter(
            ...
            c=labels[mask],
            cmap="tab20",
            s=16 if domain_name == "synthetic" else 12,
            marker=marker,
            alpha=alpha,
            linewidths=0.5,
            label=domain_name,
        )
    plt.title(f"t-SNE: {args.dataset}, {args.networks}, IPC={args.ipc}, epoch={epoch}")
    ...
    try:
        wandb.log({"tsne/plot": wandb.Image(output_path), "tsne/epoch": epoch})
    except Exception as exc:
        print(f"Warning: failed to log t-SNE figure to wandb: {exc}")
...
def test(epoch,wandb_metrics):
    ...
    if args.tsne and epoch == start_epoch + args.epochs - 1:
        run_tsne_analysis(epoch)
